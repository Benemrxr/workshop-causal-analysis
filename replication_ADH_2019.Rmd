...
industry_shares_90 <- bartik.weight::ADH_local
```



Different variables with easier names (same as in Bartik package example)
```{r}
master <- workfile9014wwd
exposure <- sic87dd_exposure
trade_flows <- china9114
ind_shares_data <- cbp_czone_merged
ind_shares_package <- industry_shares_90
```
...
We keep the strucutre as in the example provided for the bartik weights package (https://rdrr.io/github/jjchern/bartik.weight/f/README.md)
